...
function cycle(){
	...
	currentCycleCounter++;
    //console.log(currentCycleCounter);
    //stop after 100 if meltMode is on is true
    if (currentCycleCounter>200 && meltMode==true){
	...
	for (var x=0;x<field.length;x++){
		for (var y=0;y<field[0].length;y++) {
			...
			if (field [x][y].exists===true && field[x][y].cycleCounter==currentCycleCounter) {
                ...
                if (field[x][y].red > 254) {
                    field[x][y].red = getRand(residue,250);
                    field[x][y].blue = getRand(field[x][y].blue,20);
                    field[x][y].green = getRand(field[x][y].green,20);

                }
                if (field[x][y].green > 254) {
                    field[x][y].green = getRand(residue,250);
                    field[x][y].blue = getRand(field[x][y].blue,20);
                    field[x][y].red = getRand(field[x][y].red,20);

                }
                if (field[x][y].blue > 254) {
                    field[x][y].blue = getRand(residue,250);
                    field[x][y].red = getRand(field[x][y].red,20);
                    field[x][y].green = getRand(field[x][y].green,20);
                }
